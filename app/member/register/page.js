...
export default function MemberRegister() {
  ...
  const saveAndRedirect = (name) => {
    try {
      ...
      // Delay sedikit agar animasi 'Berhasil' terlihat oleh user
      setTimeout(() => {
        router.replace('/');
        router.refresh();
      }, 1000);
      ...
      alert('Gagal menyimpan sesi login. Pastikan browser tidak dalam Mode Incognito ketat.');
    }
  };
  ...
  const handleGoogleRegister = () => {
    // Simulasi langsung ambil nama dari akun Google default tanpa pop-up prompt()
    const mockGoogleName = "Budi Santoso"; 
    saveAndRedirect(mockGoogleName);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans text-slate-800">
      <div className="bg-white p-8 rounded-2xl shadow-lg max-w-md w-full border border-slate-200">
        ...
        {submitted && (
          <div className="bg-emerald-100 border border-emerald-400 text-emerald-800 text-xs p-3 rounded-lg mb-4 text-center font-bold animate-fade-in">
